...
| | |
|---|---|
| **App** | https://ml-lngmarketinsight.streamlit.app/ |
| **Source** | https://github.com/paust07-gif/20251129 |
...
|---|---|---|
| **S&P Global Commodity Insights** | `AAOVQ00`, `CN06J` | JKM spot price, JKM forward curve (M+1~M+24) |
| **S&P Global LNG Analytics** | Asia Spot Forecast | Monthly LNG price forecast |
| **FRED (St. Louis Fed)** | `DHHNGSP` | Henry Hub natural gas daily spot price |
| **FRED** | `DCOILBRENTEU`, `DCOILWTICO` | Brent / WTI crude oil spot price |
| **Yahoo Finance** | `TTF=F`, `EURUSD=X` | TTF natural gas futures, EUR/USD FX rate |
...
1. **FRED `DHHNGSP`** (primary) — EIA daily spot, most accurate; captures Winter Storm Fern Jan 26, 2026 peak (~$30.57/MMBtu)
2. **Yahoo Finance `NG=F`** removed from HH pipeline — front-month futures dilute spot spikes
3. **`sample_spot()` fallback** — date-aware realistic pattern encoding the Jan 2026 surge when API unavailable
...
```bash
git clone https://github.com/paust07-gif/20251129.git
cd 20251129
...
KT Kim (Finanacial Data Science / 2025510105)
